...
很多人相信下一个时代的主题是智能化，它要求机器充当人的部分功能，像人那样敏捷的处理各种类型的复杂问题。在人类行为因数字化的普及而被各种形式的数据记录下来的背景下，机器学习被认为是实现智能化的各种途径中很有希望的一个。智能化任务可以被分为感知和决策两个阶段，支撑人类决策的感知数据通常是非结构化的，主要包括语音和图像，本文主要讨论后者。图像表达的是视觉信息是人类获取信息的最主要途径。在带来丰富信息的同时，图像生成过程的复杂性也给自动化分析带来了困难，因为我们不可能对其正向建模。这给传统机器学习算法带来了挑战传统的机器学习算法假设处理的是结构化数据，即定义良好、意义明确的特征向量。，它要求我们使用基于领域知识的特征工程将数据转换为结构化的，这只能在一定程度上解决问题。就图像数据为例，研究人员几十年来提出了各种特征提取方法，代表性的SIFT算子能够捕捉到图像局部的具有旋转和尺度不变性的特征点，然而这样的底层特征不足以满足智能化任务中的高层信息需求。深度学习技术扩展了传统机器学习的适用范围，它尝试以自动学习特征转换替代显式的特征工程，构建一个端到端的学习系统。它可以从具有复杂结构和极大灵活性的数据中发现和任务相关的重要因子，因此深度学习又被称为表征（representation）学习。数据转换是由一系列简单的非线性变换构成的，针对不同类型的数据，变换单元的基本结构也不一样。本文仅讨论针对图像数据设计的卷积神经网络模型。
