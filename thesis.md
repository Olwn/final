...
其中，$I$表示一幅图像或一个特征图，$m$是在每个通道上单独求得的均值，$f$是调整比率。$f>1$时图像的对比度会被增强，$f<1$时减弱，$f=1$时扩增操作不变产生任何改变。实验中，各个通道的$f$是独立且都服从同一个均匀分布的。每次前向计算时使用的$f$是抽样得到的，$f>1$的概率等于$f<1$的概率，并且期望值是$1$。这意味着同一层的某些通道的对比度会被增强，而另一些被减弱，并且所有通道的累加会保持大致相等。我们也尝试了增强所有特征图的对比度，但是得到了更差的测试集识别率，这可能是因为扩增操作在训练集上引入了一个数值偏差。$\alpha$被经验性的设置为$0.25$。下面的图片展示了分别在原始图像和特征图上做对比度变换的效果。
