...
很多人相信下一个时代的主题是智能化，它要求机器完成人的部分工作，像人那样敏捷的处理各种类型的复杂问题。在人类行为因数字化的普及而被各种形式的数据记录下来的背景下，机器学习被认为是实现智能化的各种途径中很有希望的一个。智能化任务可以被分为感知和决策两个阶段，支撑人类决策的感知数据通常是非结构化的，主要包括语音和图像，本文的讨论仅限于后者。图像表达的视觉信息是人类获取信息的最主要途径。在带来丰富信息的同时，图像生成过程的复杂性也给自动化分析带来了困难。我们不可能对其正向建模，因为那涉及到模拟整个宇宙。对于传统的机器学习，图像处理也是困难的。传统机器学习算法假设数据是结构化的，即定义良好、意义明确的特征向量。在解决实际问题时，需要利用领域知识将原始图像数据转换为向量，这只能在一定程度上解决问题。图像处理的研究人员几十年来提出了各种特征提取方法，代表性的SIFT算子能够捕捉到图像局部的具有旋转和尺度不变性的特征点，然而这样的底层特征不足以满足智能化任务中的高层信息需求。深度学习尝试以自动学习特征替代传统的的人工特征，构建端到端的学习系统。它可以从具有极大灵活性的数据中发现和任务相关的重要因子，因此深度学习又被称为表征（representation）学习。深度学习使用的模型是神经网络，专为图像数据设计的神经网络被称为卷积神经网络。它主要由卷积层和pooling层组成，卷积层的卷积核在图像的每个位置扫描以提取特征，pooling层合并相邻的特征。卷积和pooling多次交替出现的结构使得卷积神经网络是一个层次化的特征提取器，前面层的输出特征是后面层的输入数据。现在比较流行的卷积神经网络模型具有十几到数十层的结构，这一方面带来了表达力的提升，另一方面引入了过拟合的风险。

在Xavier初始化方法和Batch-Norm被提出后，深层次的卷积神经网络的训练已经不再是难题。然而，训练结果只代表了模型在训练集上的表现，而模型的价值体现在它在测试集上的表现。两者的差异可用来衡量泛化性，在训练误差都接近0的前提下，泛化性几乎就代表了模型的实际表现。因此，提升模型的泛化性十分重要，专为此目标而设计的方法被称为正则化方法，这也是本文的研究对象。

#### 1.2 正则化方法概述

无论是在上世纪八九十年代的神经网络研究热潮，还是近几年的深度学习的兴起，正则化方法都是研究的重点。[7]长期以来，参数衰减被作为提高泛化性能的技巧使用于神经网络的训练中。Anders Korgh等人对这种方法做了理论上的分析和实验上的验证，他们以最简单的神经网络-感知机-为例，推导出参数衰减可以抑制不重要变量的权重，并且在强度合适的情况下可以阻止拟合目标变量中的随机噪声，它从这两方面提高模型的泛化性能。对于含有隐含层和非线性单元的网络，他们在局部线性的假设下推导出参数衰减使得训练收敛于范数更小的参数。之后很多年，参数衰减都是神经网络正则化的标准方法。深度学习兴起后，有许多针对深层次神经网络的正则化方法被提出，然而这些方法更多的是基于直觉和实验，缺乏理论分析。这其中较早出现、也比较最具代表性的是Nitish等人提出的Dropout，它在网络训练阶段随机的使得某些节点的输出为0，就好像将这些节点从网络中去掉了一样，在测试阶段它们又和正常的节点一样工作，大量实验验证了它能够显著提高泛化性能。作者将其解释为一种高效的模型集成的实现，因为每次迭代都是计算的不同结构的子网络，测试阶段的结果近似于这些子网络的输出的平均。然而作者本人在论文中提到将Dropout换成高斯噪声也可以达到接近的效果，Wager等人以GLM模型为基础推导出Dropout隐式的施加了$L2$正则。Li Wan等人对Dropout做了扩展而提出了DropConnect，它随机的将某些连接参数设置为0，这样同层的不同节点接收到的是前一层节点的不同子集的信号。它允许产生更多的子网络，因为参数的数量远多于节点数量，但从实验室结果看，它相比Dropout没有显著优势。Batch-Norm被设计用于减少节点输出的分布的偏移，但被发现同时具有显著的正则化效果。和Dropout的相似之处在于，它也在训练阶段向节点输出加入了随机性。Batch-Norm引入了一个线性变换，但变换的参数是在每个训练批量上求得的，因为批量选取的随机性，参数可被看成随机变量。尽管看起来它们的原理不同，但是简单的同时使用Dropout和Batch-Norm并不能获得叠加的效果。随着Batch-Norm因对深层网络的训练的积极作用而被广泛使用，Dropout的重要性正在降低。

可能是受到Dropout和Batch-Norm的启发，研究人员开始意识到训练阶段的随机性和测试阶段的泛化性之间存在关联，之后有很多利用了随机性的正则化方法被提出。Lingxi Xie等人提出的DistrubLabel，训练时随机的将每个训练批次中的某些样本的类别替换为错误的。Gao Huang等人受到残差网络的启发提出的Stochastic Depth，训练时随机的跳过某些层而将不相邻的层直接连接，测试时正常计算所有层构成的网络。Saurabh Singh等人将Dropout、Stochastic Depth和残差网络结合起来，训练时随机的选择某种连接，测试时抽样几十种确定的网络结构然后计算它们的输出的均值。Guoliang Kang等人也扩展了Dropout提出Shakeout，它在训练时随机的加强或者减弱某些参数，作者以GLM模型为例推导了Shakeout同时具有$L0$，$L1$和$L2$的正则化效果。

以上提到的都是通过修改训练阶段的梯度更新方式而实现正则化的方法，还有另一类简单有效的提升泛化性能的方法：数据扩增。这类方法的原理非常直观，用已有数据合成新的数据，更多的数据可以更好的描述函数的形状和更好的近似数据的真实分布（测试集分布）。稍微严格的说，数据扩增只是可能带来泛化性能提升，通常是很微弱的提升，因为生成的数据和原数据是高度相关的。 对于图像数据类型，有很多种扩增的手段。随机裁剪是最常见的，随机翻转在不改变目标语义的情况下也可以使用，这是大多数文献中会出现的两种图像扩增方法。调整图像的角度、比例、对比度和亮度等指标也可以生成新的图像，但是在前面两种方法存在的情况下，额外施加这些扩增操作带来的性能提升非常微弱，因此他们较少被使用。最近，一些工作发现在图像中加入干扰性的噪声也可以起到数据扩增的效果。PathShuffle随机的改变图像局部块的排列，RandomErasing在图像里随机的选择一个矩形区域然后将其填充为随机值。

#### 1.3 论文结构

后面的内容包含三个章节，章节二介绍了理解本文所必需的预备知识，包括神经网络构建、神经网络训练、模型容量和泛化性能等概念。章节三单独分析了几种最为重要的正则化方法，基本的分析思路是先介绍概念再以实验测试不同强度的正则化的效果。在可能的情况下，本文还设置实验探究方法的原理，特别是对于Dropout和基于随机裁剪的数据扩增，我们有新的不同于原论文或者直觉的结论。章节四引入了特征扩增层的概念，这是一种新颖的正则化方法。章节五是对本文的总结。
...
| -------- | :---------: | :---------: | :------------------: | :--------------------: | :-------------------------: |
| 8-layer  | 0.46(11.6%) | 0.48(13.2%) |     0.51(11.9%)      |      0.43(11.2%)       |       **0.42(11.7%)**       |
| 14-layer | 0.45(9.00%) | 0.45(9.16%) |     0.46(9.52%)      |      0.41(8.48%)       |       **0.41(8.62%)**       |
| 20-layer | 0.47(8.45%) | 0.49(7.92%) |     0.48(8.78%)      |      0.44(7.93%)       |       **0.40(7.54%)**       |
| 32-layer | 0.45(7.70%) | 0.59(7.99%) |     0.43(7.38%)      |      0.43(7.07%)       |       **0.41(7.10%)**       |
| 44-layer | 0.45(7.03%) | 0.58(7.44%) |     0.47(7.24%)      |      0.42(6.37%)       |       **0.39(6.54%)**       |

表2

| config   | normal           | image-<br />bright | feature-<br />bright | feature-<br />bright-more |
| -------- | ---------------- | ------------------ | -------------------- | ------------------------- |
| 8-layer  | 0.451(11.70%)    | 0.449(11.6%)       | 0.442 (11.6%)        | **0.438 (11.2%)**         |
| 14-layer | 0.450(9.90%)     | 0.475(9.79%)       | **0.437 (9.04%)**    | 0.447 (9.09%)             |
| 20-layer | 0.473(8.33%)     | 0.464 (9.36%)      | 0.457 (8.35%)        | **0.446 (7.83%)**         |
| 32-layer | 0.457(7.67%)     | 0.476 (7.82%)      | 0.447 (7.22%)        | **0.445 (7.16%)**         |
| 44-layer | **0.432(6.91%)** | 0.442 (7.20%)      | 0.445 (6.82%)        | 0.438 (6.77%)             |
