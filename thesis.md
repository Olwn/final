...
dropout、batch norm、Disturblabel、Stochastic Depth、swapout、Shakeout
...
以上提到的都是通过修改训练阶段的梯度更新方式而实现正则化的方法，还有另一类简单有效的提升泛化性能的方法：数据扩增。这类方法的原理非常直观，更多的数据可以更好的描述函数的形状，更多的数据可以更好的近似数据的真实分布（测试集分布）。稍微严格的说，数据扩增只是可能带来泛化性能提升，通常是很微弱的提升，因为生成的数据和原数据是高度相关的。 对于图像数据类型，有很多种扩增的手段。随机裁剪是最使用的，随机翻转在不改变目标语义的情况下也可以使用。

Data Aug: crop, PatchShuffle, RandomErasing
